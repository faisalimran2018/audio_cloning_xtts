...
def split_text_max_chars(text: str, max_chars: int) -> List[str]:
    ...
    # Hard-split long chunks by words if still too long
    final: List[str] = []
    for c in chunks:
        if len(c) <= max_chars:
            final.append(c)
...
def main():
    ...
    ap.add_argument("--language", default="ar", help="Language code, e.g. ar, en, ur")
    ap.add_argument("--text_key", default="text_ar", help="Manifest field to TTS (e.g. text_ar, en_text, ur_text)")
    ...
    for s in segments:
        text = normalize_spaces(s.get(args.text_key) or "")

        # Resolve paths relative to manifest directory
        speaker_wav = (base_dir / s["chunk_wav"]).resolve()
        out_wav = (base_dir / s["tts_out_wav"]).resolve()
        ...
        parts = split_text_max_chars(text, args.max_chars)
        part_wavs: List[Path] = []
        ...
        try:
            ...
            # multi-part: generate each part then concat with pauses
